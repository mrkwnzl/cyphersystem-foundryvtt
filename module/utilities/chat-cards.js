export function chatCardMarkItemIdentified(actor, item) {
  let content = game.i18n.format("CYPHERSYSTEM.PCAskingForIdentification", {actor: actor.name}) + `<div class='chat-card-buttons'><a class='confirm' data-item='${item._id}' data-actor='${actor.id}'><i class="fa-item fas fa-check"></i> ${game.i18n.localize("CYPHERSYSTEM.Confirm")}</a></div>`;
...
export function chatCardAskForIntrusion(actor, actorId) {
  let refuseButton = (actor.system.basic.xp > 0) ? ` | <a class='refuse-intrusion ' data-actor='${actorId}'><i class="fa-item fas fa-times"></i> ${game.i18n.localize("CYPHERSYSTEM.Refuse")}</a>` : "";

  let content = game.i18n.format("CYPHERSYSTEM.ProposingIntrusion", {actor: actor.name}) + `<div class='chat-card-buttons' data-actor='${actorId}'><a class='accept-intrusion' data-actor='${actorId}'><i class="fa-item fas fa-check"></i> ${game.i18n.localize("CYPHERSYSTEM.Accept")}</a>` + refuseButton + "</div>";
